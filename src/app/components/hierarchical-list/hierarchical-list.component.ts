import { Component, computed, input, signal, effect, OnDestroy, HostListener, ElementRef, inject } from '@angular/core';
import {
  DataRecord, GenericColumnDef, HierarchyConfig, TreeNode, DEFAULT_HIERARCHY,
} from '../../models/hierarchy.model';
import { buildTree, applyAggregates } from './tree-builder';
import {
...
@Component({
    ...
    <div class="forecast-table" [class.is-resizing]="resizing()">
      ...
      <div class="table-body">
        @for (row of visibleRows(); track row.node.id) {
          ...
          >
            <!-- Name column (always first, special) -->
            <div
              class="col-name"
  ...
  styles: [`
    :host {
      ...
      --color-label: #757575;
      --color-error: #D2310D;
      --color-success: #0B8500;
      --color-border-row: #E3E3E3;
      --color-border-cell: #E7E7E7;
      --color-row-selected: #E7ECFA;
      --color-progress-track: rgba(204, 222, 252, 0.5);
      position: relative;
      display: block;
      outline: none;
    }
    ...
    .forecast-table.is-resizing { cursor: col-resize; user-select: none; * { cursor: col-resize !important; user-select: none !important; } }

    .table-header, .table-row { display: grid; align-items: center; }

    .table-header {
      height: 36px; border-bottom: 1px solid var(--color-border-row); background: var(--color-background); position: sticky; top: 0; z-index: 1;
      .col-name, .col-number { font-family: 'Montserrat', sans-serif; font-size: 12px; font-weight: 500; line-height: 16px; color: var(--color-label); white-space: nowrap; position: relative; }
      .col-name { padding-left: 48px; border-right: 1px solid var(--color-border-cell); display: flex; align-items: center; justify-content: space-between; padding-right: 8px; }
      .col-number { text-align: right; padding: 8px 16px 8px 4px; }
    }

    .resize-handle { position: absolute; right: -2px; top: 0; bottom: 0; width: 5px; cursor: col-resize; z-index: 2; transition: background 0.1s; &:hover { background: var(--color-link); opacity: 0.3; } }
    .is-resizing .resize-handle:hover { background: none; }

    .header-sortable { cursor: pointer; user-select: none; transition: color 0.12s; &:hover { color: var(--color-secondary); } }
    ...
    .column-picker-btn { background: none; border: 1px solid transparent; border-radius: 4px; cursor: pointer; padding: 2px; display: flex; align-items: center; justify-content: center; transition: all 0.12s; &:hover { background: var(--color-row-selected); border-color: var(--color-border-cell); } }
    ...
    .picker-close { background: none; border: none; cursor: pointer; padding: 2px; display: flex; align-items: center; border-radius: 4px; &:hover { background: var(--color-row-selected); } }
    .picker-actions { display: flex; gap: 8px; padding: 8px 12px 4px; }
    .picker-action-btn { background: none; border: none; cursor: pointer; font-family: 'Montserrat', sans-serif; font-size: 11px; font-weight: 500; color: var(--color-link); padding: 0; &:hover { text-decoration: underline; } }
    .picker-item { display: flex; align-items: center; gap: 8px; padding: 6px 12px; cursor: pointer; font-family: 'Montserrat', sans-serif; font-size: 13px; font-weight: 500; color: var(--color-foreground); transition: background 0.1s; &:hover { background: var(--color-row-selected); } input[type="checkbox"] { width: 16px; height: 16px; accent-color: var(--color-link); cursor: pointer; margin: 0; } }

    .table-row { height: 48px; border-bottom: 1px solid var(--color-border-row); &:last-child { border-bottom: none; } > * { align-self: stretch; } }
    .parent-row { background-color: var(--color-row-selected); .node-name { font-weight: 500; } }
    ...
    .expand-btn { background: none; border: none; cursor: pointer; padding: 0; width: 16px; height: 16px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; &:hover { opacity: 0.7; } }
    .chevron-icon { transition: transform 0.15s ease; &.expanded { transform: rotate(90deg); } }
    ...
    .data-cell {
      ...
      font-family: 'Montserrat', sans-serif; font-size: 13px; font-weight: 500; line-height: 20px;
      color: var(--color-foreground); font-variant-numeric: tabular-nums; white-space: nowrap; cursor: pointer;
    }
    ...
    .progress-bar { flex: 1; height: 6px; background-color: var(--color-progress-track); border-radius: 4px; overflow: hidden; padding: 2px; }
    .progress-fill { height: 2px; background: linear-gradient(181deg, #0058EF 5%, #0042B5 103%); border-radius: 4px 0 0 4px; transition: width 0.3s ease; }
...
export class HierarchicalListComponent implements OnDestroy {
  ...
  readonly columns = input<GenericColumnDef[]>(DEFAULT_COLUMNS);
  readonly hierarchy = input<HierarchyConfig>(DEFAULT_HIERARCHY);
  readonly storageKey = input('forecast-hierarchy');

  protected allColumns = computed(() => this.columns());

  private hostEl = inject(ElementRef<HTMLElement>);
  private expandedNodes = signal(new Set<string>());
  protected visibleColumns = signal<Set<string>>(new Set());
  protected columnPickerOpen = signal(false);
  protected sortState = signal<SortState | null>(null);
  protected columnWidths = signal<Record<string, number>>({});
  ...
  protected selectedCell = signal<CellRef | null>(null);
  protected copyToast = signal<string>('');
  private toastTimeout: ReturnType<typeof setTimeout> | null = null;

  private treeRoots = computed(() => {
    const roots = buildTree(this.data(), this.hierarchy());
    applyAggregates(roots, this.columns());
    return roots;
  });

  protected visibleColumnDefs = computed(() => {
    const visible = this.visibleColumns();
    return this.allColumns().filter(c => visible.has(c.id));
  });

  protected gridColumns = computed(() => {
    const widths = this.columnWidths();
    const nameWidth = widths['__name__'] ?? NAME_DEFAULT_WIDTH;
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (!this.storageInitialized) {
        ...
        this.visibleColumns.set(
          storedVis.length > 0 ? new Set(storedVis) : new Set(cols.map(c => c.id))
        );
        this.columnWidths.set(loadFromStorage<Record<string, number>>(`${key}-column-widths`, {}));
      }
    });

    effect(() => {
      const roots = this.treeRoots();
      if (roots.length > 0 && this.expandedNodes().size === 0) {
        const ids = new Set<string>();
        this.collectParentIds(roots, ids);
    ...
    effect(() => {
      const visible = this.visibleColumns();
      const key = this.storageKey();
      if (this.storageInitialized && visible.size > 0) {
        localStorage.setItem(`${key}-visible-columns`, JSON.stringify([...visible]));
      }
    });
    ...
    effect(() => {
      ...
      const key = this.storageKey();
      if (this.storageInitialized) {
        localStorage.setItem(`${key}-column-widths`, JSON.stringify(widths));
      }
    });
  }
  ...
  protected visibleRows = computed(() => {
    ...
    this.walk(this.treeRoots(), 0, expanded, sort, rows);
    return rows;
  });

  // --- Template helpers ---

  protected getField(record: DataRecord, field: string): string {
    const val = record[field];
    return val == null ? '' : String(val);
  }

  protected isStatusSuccess(record: DataRecord): boolean {
    const h = this.hierarchy();
    if (!h.statusField) return false;
    const val = String(record[h.statusField] ?? '');
    return h.statusValues?.success?.includes(val) ?? false;
  }

  protected isStatusError(record: DataRecord): boolean {
    const h = this.hierarchy();
    if (!h.statusField) return false;
    const val = String(record[h.statusField] ?? '');
    return h.statusValues?.error?.includes(val) ?? false;
  }

  protected getCellDisplay(record: DataRecord, col: GenericColumnDef): string {
    return formatCellValue(record[col.field], col);
  }

  protected getProgress(record: DataRecord, col: GenericColumnDef): number {
    return computeProgress(record, col) ?? 0;
  }

  protected getTrend(record: DataRecord, col: GenericColumnDef): TrendDirection | null {
    return computeTrend(record, col);
  }

  protected getValueClasses(record: DataRecord, col: GenericColumnDef): string {
  ...
  protected getTrendColorClass(trend: TrendDirection, col: GenericColumnDef): string {
  ...
  onKeydown(event: KeyboardEvent): void {
    ...
    if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) {
      if (!cell) return;
      event.preventDefault();
      this.moveSelection(cell, event.key as any);
    }
  }
  ...
  private moveSelection(cell: CellRef, direction: string): void {
    ...
    this.selectedCell.set({ nodeId: rows[newRow].node.id, columnId: columns[newCol] });
  }

  protected selectCell(nodeId: string, columnId: string): void {
    this.selectedCell.set({ nodeId, columnId });
    this.hostEl.nativeElement.focus();
  }

  protected isCellSelected(nodeId: string, columnId: string): boolean {
    const cell = this.selectedCell();
    return cell !== null && cell.nodeId === nodeId && cell.columnId === columnId;
  }

  protected onExpandClick(event: MouseEvent, nodeId: string): void {
  ...
  private getCellCopyValue(cell: CellRef): string | null {
    ...
    if (!row) return null;
    if (cell.columnId === '__name__') {
      return this.getField(row.node.record, this.hierarchy().nameField);
    }
    const col = this.allColumns().find(c => c.id === cell.columnId);
    if (!col) return null;
  ...
  private showCopyToast(value: string): void {
    ...
    this.copyToast.set(display);
    this.toastTimeout = setTimeout(() => this.copyToast.set(''), 1500);
  }

  // --- Resize ---

  protected onResizeStart(event: MouseEvent, columnId: string): void {
    event.stopPropagation();
    event.preventDefault();
    ...
    if (columnId === '__name__') {
      this.resizeStartWidth = widths['__name__'] ?? NAME_DEFAULT_WIDTH;
      this.resizeMinWidth = NAME_MIN_WIDTH;
    } else {
      const col = this.allColumns().find(c => c.id === columnId);
      this.resizeStartWidth = widths[columnId] ?? col?.defaultWidth ?? 140;
  ...
  private onResizeMove(event: MouseEvent): void {
    ...
    this.columnWidths.update(w => ({ ...w, [this.resizeColumnId!]: newWidth }));
  }
  ...
  protected resetColumnWidths(): void { this.columnWidths.set({}); }
  ...
  protected toggleColumn(columnId: string): void {
    this.visibleColumns.update(set => { const next = new Set(set); next.has(columnId) ? next.delete(columnId) : next.add(columnId); return next; });
  }
  protected showAllColumns(): void { this.visibleColumns.set(new Set(this.allColumns().map(c => c.id))); }
  protected hideAllColumns(): void { this.visibleColumns.set(new Set()); }
  protected openColumnPicker(event: MouseEvent): void { event.stopPropagation(); this.columnPickerOpen.update(v => !v); }
  protected toggleColumnPicker(): void { this.columnPickerOpen.update(v => !v); }

  // --- Expand/collapse ---

  protected toggleExpand(nodeId: string): void {
    this.expandedNodes.update(set => { const next = new Set(set); next.has(nodeId) ? next.delete(nodeId) : next.add(nodeId); return next; });
  }
  ...
  private sortNodes(nodes: TreeNode[], sort: SortState): TreeNode[] {
    const field = sort.columnId === '__name__' ? this.hierarchy().nameField
      : this.allColumns().find(c => c.id === sort.columnId)?.field;
